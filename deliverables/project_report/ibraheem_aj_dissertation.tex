...
\chapter{Machine Learning Classifiers}
	...
	\section{Machine Learning Models}
		...
		The ensemble classifier achieved an accuracy of $\sim94\%$ on both the simple molecular and the fingerprint dataset with its learning curve shown in figure ; Which further enforces the claim by Thomas G. Dietterich \cite{Dietterich} that ensemble learners perform better than individual learners.
			\begin{figure}[H]
				\centering
...
\chapter{Model Evaluation and Persistence}
	\section{Model Evaluation}
		...
		A confusion matrix shows a detailed breakdown of the miscalculations of a machine learning model and it is sometimes known as an error matrix. It highlights in a table of two dimensions (\textit{actual} and \textit{predicted}) where the model made a correct and wrong prediction. The following terms are used when describing the parameters of a confusion matrix:
			\begin{itemize}
		...
		With these parameters, we can further calculate certain observations about the model such as its precision and recall, explained below
			...
			The ROC curve is illustrated by plotting the True Positive Rate (TPR; The fraction of true positives out of the positives) against the False Positive Rate (FPR; The fraction of False positives out of the negatives) at various thresholds \cite{TFawcett2005}. Shown in figure \ref{fig:roc} is the ROC curve of the Ensemble classifier when evaluated with the simple molecular descriptor dataset.
				\begin{figure}[H]
					\centering
